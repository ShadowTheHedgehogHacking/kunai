...
namespace libWiiSharp
{
    ...
    public class TPL : IDisposable, IBinarySerializable
    {
        ...
        public void Read(BinaryObjectReader reader)
        {
            ...
            int magicBf = reader.ReadInt32();
            if (magicBf != (int)tplHeader.TplMagic)
            {
                throw new Exception("TPL Header: Invalid Magic!");
            }
            tplHeader.NumOfTextures = (uint)reader.ReadInt32();
            int headerSize = reader.ReadInt32();
            if (headerSize != (int)tplHeader.HeaderSize)
            {
            ...
            for (int index = 0; index < tplHeader.NumOfTextures; ++index)
            {
                ...
                tplTextureEntry.TextureHeaderOffset = (uint)reader.ReadInt32();
                tplTextureEntry.PaletteHeaderOffset = (uint)reader.ReadInt32();
            ...
            for (int index = 0; index < tplHeader.NumOfTextures; ++index)
            {
                ...
                reader.Seek(tplTextureEntries[index].TextureHeaderOffset, SeekOrigin.Begin);
                var size = reader.ReadArray<byte>(4);
                tplTextureHeader.TextureHeight = BitConverter.ToUInt16(size, 0);
                tplTextureHeader.TextureWidth = BitConverter.ToUInt16(size, 2);
                tplTextureHeader.TextureFormat = (uint)reader.ReadInt32();
                tplTextureHeader.TextureDataOffset = (uint)reader.ReadInt32();                
                tplTextureHeader.WrapS = (uint)reader.ReadInt32();                
                tplTextureHeader.WrapT = (uint)reader.ReadInt32();                
                tplTextureHeader.MinFilter = (uint)reader.ReadInt32();                
                tplTextureHeader.MagFilter = (uint)reader.ReadInt32();                
                tplTextureHeader.LodBias = (uint)reader.ReadInt32();                
                tplTextureHeader.EdgeLod = reader.ReadByte();
                tplTextureHeader.MinLod = reader.ReadByte();
                tplTextureHeader.MaxLod = reader.ReadByte();
                tplTextureHeader.Unpacked = reader.ReadByte();
                if (tplTextureEntries[index].PaletteHeaderOffset != 0U)
                {
                    ...
                    reader.Seek(tplTextureEntries[index].PaletteHeaderOffset, SeekOrigin.Begin);
                    var buffer = reader.ReadArray<byte>(4);
                    tplPaletteHeader.NumberOfItems = BitConverter.ToUInt16(buffer, 0);
                    tplPaletteHeader.Unpacked = buffer[2];
                    tplPaletteHeader.Pad = buffer[3];
                    tplPaletteHeader.PaletteFormat = (uint)reader.ReadInt32();
                    tplPaletteHeader.PaletteDataOffset = (uint)reader.ReadInt32();
                }
                reader.Seek(tplTextureHeader.TextureDataOffset, SeekOrigin.Begin);
                ...
                buffer2 = reader.ReadArray<byte>(buffer2.Length);
                if (tplTextureEntries[index].PaletteHeaderOffset != 0U)
                {
                    //FireDebug("   Reading Palette #{1} of {2}... (Offset: 0x{0})", tplFile.Position, index + 1, tplHeader.NumOfTextures);
                    reader.Seek(tplPaletteHeader.PaletteDataOffset, SeekOrigin.Begin);
                    buffer3 = reader.ReadArray<byte>(buffer3.Length);
                }
                else
                {
                    buffer3 = new byte[0];
